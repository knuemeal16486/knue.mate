...
enum MealType {
  ...
  final String label; final IconData icon; final String timeRange; final String stdKey;
...
Future<void> _updateWidgetWithData(Map<String, dynamic> data, MealSource source) async {
  try {
    ...
    if (time < 570) { title = "오늘 아침"; menu = breakfast; }
    else if (time < 840) { title = "오늘 점심"; menu = lunch; }
    else if (time < 1170) { title = "오늘 저녁"; menu = dinner; }
    else { title = "내일 메뉴"; menu = ["내일 식단을 확인하세요"]; }

    String menuString = menu.isEmpty ? "정보 없음" : menu.map((e) => "· $e").join("\n");
    ...
    await HomeWidget.saveWidgetData<double>('transparency', widgetTransparency.value);
...
class NotificationService {
  ...
  Future<void> init() async {
    ...
    await flutterLocalNotificationsPlugin.initialize(const InitializationSettings(android: android, iOS: ios));
  }
