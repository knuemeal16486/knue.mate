    ...
    for (var k = 0; k < loops.length; k++) {
      final w = straighten([
        for (final p in loops[k])
          Pt(ox + (p.x + o[0]) * res, oy + (p.y + o[1]) * res)
      // mergeDeg 12→28. 곧은 구간을 더 과감히 한 변으로 합친다. maxOffset은
    // 1.4m 그대로 둬서 **진짜 곡선(순환도로 등)은 안 펴진다** — 합쳐진 직선이
    // 원래 점들에서 1.4m 넘게 벌어지면 합치지 않는다.
    //
    // 실측(지그재그율 / 포장면 IoU / 점 수):
    //   12,1.4  15.1%  89.1%  4943   ← 예전
    //   20,1.4  12.6%  88.6%  4526
    //   28,1.4  10.1%  88.2%  4132   ← 지금
    //   28,2.4   8.1%  87.0%  3756   (오프셋을 풀면 곡선이 각져 손해가 크다)
